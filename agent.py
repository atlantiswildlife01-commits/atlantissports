...
def fetch_youtube_cc_video(keyword: str) -> tuple[str | None, str]:
    ...
    try:
        ...
        search_cmd = ["yt-dlp", url, "--flat-playlist", "--playlist-items", "1-10",
                      "-J", "--quiet", "--no-warnings"]
        ck = _yt_cookies_file()
        if ck:
...
_QUEUE_STOP = {"the", "a", "an", "of", "in", "on", "and", "sport", "sports",
               "match", "game", "video", "footage", "hd", "4k", "action", "india", "indian"}
...
def fetch_queued_video(want_kw: str = "") -> tuple[str | None, str]:
    ...
    Isse video hamesha text/topic se match karti hai (warna cricket-text pe football-video aa jati thi)."""
